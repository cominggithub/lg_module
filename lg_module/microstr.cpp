...
void read_microstr(char *fname, local_str *lstr)
{
	...
	if (fp!=NULL)
	{
		...
		for(i=0;i<lstr->nx*lstr->ny;i++) 
		{
			...
			lstr->thz[i] = cbuf/pow(10.0,6.0);  // Note! in unit of mm; In this case, transform nm to mm
			if(zdim_out>cbuf/pow(10.0,6.0)) zdim_out = cbuf/pow(10.0,6.0);
		}
	}
	else
	{
...
void find_str_hit_local(ray_trace1 *ray1, local_str *lstr)  // change coordinate origin into the center of the top plane of the box containg a microstrcuture
{
	...
	else		// find the intersection of ray and microstructure
	{
		...
		ray1->xr = int(begi/lstr->ny)*dx+xmin; ray1->yr = (begi%lstr->ny)*dy+ymin; ray1->zr = lstr->thz[begi];
